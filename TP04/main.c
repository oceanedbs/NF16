...
int main() {
  ...
  if (a == 1) {
    ...
    int choix = 0, n = -1;
    char chaine2[100];
    dico = initDico("initial\n"); // le "\n" est mis ici car les mots récupérés
                                  // de dictionnaire.txt ont un \n à la fin
    dico = chargerABR(dico);
    while (choix != 8) {
      printf("Que voulez vous faire  \n 1-verimot ABR \n 2 - veridico ABR \n "
             "3- print parcours prefixe \n 4 - print parcours infixe \n 5- "
             "parcours parcours_postfixe \n 6- afficher sous ensemble 1 "
             "(affiche tous les mots du dictionnaire avant la chaine passée en "
             "argument)\n 7-afficher sous ensemble 2 (affiche les mots "
             "commençant par le préfixe passé en argument)\n 8-quitter \n");
      scanf("%d", &choix);
    ...
    while (choix != 5) {
      ...
      scanf("%d", &choix);
      if (choix == 5) {
        supprimeALtotal(dico);
        break;
      }
      if (choix == 1) {
        for (lol = 0; lol < 100; lol++) {
          tab2[lol] = 0;
        }
        lol = -1;
        print(dico, tab2, 0, &lol);
      }
      if (choix == 2) {
